# danbooru_most_view.py
# 某日浏览量榜（/explore/posts/viewed）抓取，结构与 danbooru_popular.py 一致：
# 全部热数据（log/stats/viewer/drawer/need_update）走 DanbooruData → deck.db + JSON 镜像，
# 不再直接读写 log.json / artist_stats.json / viewer_data.json / drawer 文件。
import os
...
# 命令行不给 --date 时默认抓今天；历史日期可 --date 2026-03-11
...
def grabber(db_data, page_num, log_callback=None, filter_tags=['furry','futanari','guro']):
    ...
    try:
        custom_print(f"正在获取 viewed 榜第 {page_num} 页（date={db_data.today_str}）...")
        posts = danbooru_api.get_viewed_posts(db_data.today_str, page_num)
...
def run(date_str, page_start=1, page_end=1):
    db_data = DanbooruData(date_str)
    n = page_start
...
if __name__ == "__main__":
    ...
    parser.add_argument("--date", default=DEFAULT_DATE,
                        help=f"榜单日期 YYYY-MM-DD，默认今天（{DEFAULT_DATE}）")
    parser.add_argument("--start", type=int, default=1)
    parser.add_argument("--end", type=int, default=1)
    args = parser.parse_args()

    run(args.date, args.start, args.end)
